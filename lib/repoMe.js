...
exports.newRepo = async (job, app) => {
  ...
  if (validTemplate(job)) {
    ...
    if (typeof newRepository !== 'undefined' && newRepository.data.id) {
      ...
      if (process.env.GHE_HOST) {
        gitEmail = appUser + '@users.noreply.' + process.env.GHE_HOST
      } else {
        gitEmail = appUser + '@users.noreply.github.com'
      }
      try {
        ...
        await shell.exec('git config --global user.name "' + appUser + '"')
        await shell.exec('git config --global user.email "' + gitEmail + '"')
